...
ReelWriter::reuse_existing_asset(DCPAsset const& existing_asset, shared_ptr<Job> job)
{
	...
	case DCPAsset::Type::AUDIO:
		_sound_asset = make_shared<dcp::SoundAsset>(new_asset_filename);
		break;
	default:
		break;
	}
...
void
ReelWriter::setup_audio(vector<DCPAsset> const& reusable_assets, shared_ptr<Job> job, dcpomatic::DCPTimePeriod period, dcp::Standard standard)
{
	...
	auto reusable_iter = std::find_if(reusable_assets.begin(), reusable_assets.end(), [period](DCPAsset const& asset) {
		return asset.period() == period && asset.type() == DCPAsset::Type::AUDIO;
	});
	...
	, _font_metrics(film()->frame_size().height)
{
	...
	setup_audio(reusable_assets, job, period, standard);
}
...
ReelWriter::finish(boost::filesystem::path output_dcp)
{
	...
	if (_sound_asset_writer && _sound_asset) {
