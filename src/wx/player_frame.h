...
public:
	...
	explicit PlayerFrame(bool auto_play);
	~PlayerFrame();
	...
	void load_dcp(boost::filesystem::path dir, boost::optional<boost::filesystem::path> kdm = {});
	...
	void idle();
	void add_kdms(std::vector<boost::filesystem::path> const& paths);
