...
PlayerFrame::PlayerFrame(bool auto_play)
	...
	, _auto_play(auto_play)
{
	...
	_stress.LoadDCP.connect(boost::bind(&PlayerFrame::load_dcp, this, _1, boost::none));
...
void
PlayerFrame::load_dcp(boost::filesystem::path dir, optional<boost::filesystem::path> kdm)
{
	try {
		_stress.set_suspended(true);

		/* Handler to set things up once the DCP has been examined */
		auto setup = [this, kdm](weak_ptr<Job> weak_job, weak_ptr<Content> weak_content)
		{
...
void
PlayerFrame::add_kdms(vector<boost::filesystem::path> const& paths)
{
...
PlayerFrame::setup_internal_player_server()
{
	try {
		...
		server->LoadDCP.connect(boost::bind(&PlayerFrame::load_dcp, this, _1, boost::none));
