...
def make_control(debian_version, filename, debug, gui, name, cpu):
    architecture = "amd64" if cpu == "x86" else "arm64"
    f = open(filename, 'w')
    ...
    print(f'Architecture: {architecture}', file=f)
...
def package_debian(target, cpu, version, options):
    ...
    make_control(target.version, 'debian/control', target.debug, options['gui'], name, cpu)
    ...
    with open('debian/files', 'w') as f:
        ...
        arch = 'amd64' if cpu == 'x86' else 'arm64' if cpu == 'arm' else None
...
def package_rpm(target, cpu, version, options):
    ...
    arch = "x86_64" if cpu == "x86" else "aarch64" if cpu == "arm" else None
    for p in glob.glob('%s/RPMS/%s/*.rpm' % (topdir, arch)):
...
def package(target, version, options):
    ...
    if target.platform == 'windows':
        ...
        if target.detail == 'appimage':
            ...
            elif target.distro in ('centos', 'fedora', 'mageia', 'rocky'):
